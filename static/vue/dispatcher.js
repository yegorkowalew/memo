...
Vue.component('ongraph-item', {
  props: ['ongraph'],
  // template:'<li>{{ ongraph.name }}</li>'
  // template: '<li class="d-flex no-block card-body border-top"><i class="fas fa-user-circle w-30px m-t-5 text-danger"></i><div><a v-bind:href="ongraph.url" class="m-b-0 font-medium p-0">{{ ongraph.fullname }}</a><span class="text-muted"></span></div><div class="ml-auto"><div class="tetx-right"><span class="text-muted font-16">Диспетчер</span><h5 class="text-muted m-b-0">{{ ongraph.user_no }}</h5></div></div></li>'
  template: `<div>
  <div class="d-flex no-block align-items-center m-t-25">
      <span><a href="">{{ ongraph.name }}:</a>
  ...
  <div class="progress">
    <div class="progress-bar progress-bar bg-info" role="progressbar" v-bind:style="{width:ongraph.orders_percent}+'%'"></div>
  </div>
</div>`
})
